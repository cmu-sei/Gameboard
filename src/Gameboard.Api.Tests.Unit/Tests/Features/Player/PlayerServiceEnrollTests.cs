...
public class PlayerServiceEnrollTests
{
    ...
    [Theory, GameboardAutoData]
    public async Task Enroll_WithExistingPracticeModeRegistration_DoesntThrow(string gameId, string userId, IFixture fixture)
    {
        // given
        var gameStore = A.Fake<IGameStore>();
        A
        ...
        // when/then
        await Should.NotThrowAsync(() => sut.Enroll(request, actor, CancellationToken.None));
    }
}
